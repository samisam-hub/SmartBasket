...
export default function AuthCallback(){const params=useLocalSearchParams<{code?:string;error?:string}>(),started=useRef(false),[error,setError]=useState<string|null>(null);
 useEffect(()=>{if(started.current)return;started.current=true;void(async()=>{
  ...
  router.replace({pathname:'/account',params:{mode:'password'}});
 })().catch(e=>setError(e.message));},[params.code]);
