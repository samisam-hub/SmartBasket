...
export function isMealPlan(v: unknown): v is MealPlan {
  ...
  return p.version==='1'&&['review','confirmed'].includes(p.status)&&[3,5,7,14].includes(p.planningDays)&&Number.isInteger(p.householdSize)&&p.householdSize>=1&&p.householdSize<=10&&
