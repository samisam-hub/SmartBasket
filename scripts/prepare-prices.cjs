...
const {estimatePackagePrice}=require('../services/pricing/priceEstimator.ts');
(async()=>{
 ...
 for(const p of products){const e=estimatePackagePrice(p);if(e)updates.push({id:p.id,price:e.priceEstimate,confidence:e.priceConfidence,version:e.priceEstimateVersion});else if(p.priceEstimate===null)missing.push({id:p.id,name:p.name,category:p.category,quantity:p.quantityLabel,size:p.packageSize,unit:p.packageUnit});}
